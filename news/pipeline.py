...
class NewsPipeline:
    ...
    def run(self, markets: Sequence[Market | str], *, limit: int, cache: PipelineCache) -> PipelineResult:
        ...
        with ThreadPoolExecutor(max_workers=max(2, len(self.adapters))) as executor:
            future_map = {executor.submit(adapter.fetch, criteria, now=now): adapter for adapter in self.adapters}
    ...
    def _post_process(self, items: List[NewsItem], limit: int) -> List[NewsItem]:
        ...
        for item in deduped:
            ...
            scored.append(item)
        scored.sort(key=lambda itm: itm.relevance_score or 0, reverse=True)
        return scored[:limit]
